...
def grabEntryData(feed):
    ...
    for index in range(len(feed.entries)):
        #__if the number of entries stored is equal to the max, end the loop
        if len(entry_data) == entry_max:
            break
...
def getFileName(downloadUrl):
    #__declare variables
    fileName = "" #__used to store the file's name
    endOfName = len(downloadUrl) - 1 #__initially stores the position of the last character in the url, later used to find where the file name begins
    #__repeat for every character in the url
    for index in range(endOfName):
        #__check if the next character is a forward slash
        if downloadUrl[endOfName] == "/":
            #__set the file name to every character after the forward slash
            fileName = downloadUrl[endOfName+1:]
            #__end the loop
            break
        #__otherwise, go back a character
        else:
            endOfName -= 1
    #__return the file's name
    return fileName
...
def generatePage(entry_data, pfpName, userlink, username):
    ...
    for index in range(len(entry_data)):
        ...
        html_lines += f'''<img src="{pfpName}" class="pfp"><h3><a href="{userlink}" target="_parent">{username}</a></h3>
        <h5>{entry_data[index]["published"]}</h5>
        <p>{entry_data[index]['content']}</p>
        <p><a href="{entry_data[index]["entrylink"]}" target="_parent">Link to post</a></p><hr>\n'''
    #__end page
    html_lines += f'<a href="{userlink}" target="_parent">View older posts</a>\n</body>\n</html>'
...
def entryEditing(entry_data,index):
    ...
    #__remove the span tag from the end of the content
    ...
    for x in range(len(entry)):
        ...
        #__if there aren't any media tags, check to see if the current entry begins with http:// or https://
            ...
            entry[x] = f'<a href="{entry[x]}" target="_parent">{entry[x]}</a>'
